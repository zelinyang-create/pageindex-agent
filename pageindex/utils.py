...
async def generate_node_summary(node, model=None):
    is_parent = bool(node.get('nodes'))
    if is_parent:
        prompt = f"""你是一个文档分析助手。以下是一个文档章节的内容，该章节包含若干子章节。
请综述该章节的整体主题，并简要说明各子章节分别侧重哪些内容。
...
直接返回摘要，不要包含任何其他文字。"""
        ...
        prompt = f"""你是一个文档分析助手。以下是一个文档章节的具体内容。
请总结该章节的核心要点，列出最重要的具体内容、步骤或数据。
...
def generate_doc_description(structure, model=None):
    prompt = f"""你是一个文档描述生成专家。
以下是一份文档的结构信息，请为该文档生成一句话的中文描述，使该文档易于与其他文档区分。
